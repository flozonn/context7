...
Visit the [Kiro MCP Server Catalog](https://kiro.dev/docs/mcp/servers/) or click the button below to install Context7 automatically:

<a class="group relative overflow-hidden font-medium rounded-[16px] transition-all duration-350 opacity-100 text-[14px] px-4 bg-purple-500 text-white hover:text-black-900 justify-center items-center gap-2 flex whitespace-nowrap no-underline py-1.5 leading-[1.5]" aria-label="Install Add to Kiro MCP server" href="kiro://kiro.mcp/add?name=context7&amp;config=%7B%22command%22%3A%22npx%22%2C%22args%22%3A%5B%22-y%22%2C%22%40upstash%2Fcontext7-mcp%22%5D%2C%22disabled%22%3Afalse%2C%22autoApprove%22%3A%5B%5D%7D"><span class="relative z-20 flex justify-center items-center gap-2 flex">+ Add to Kiro</span><span class="!w-full !max-w-full duration-[350ms] absolute inset-0 z-10 translate-y-[50%] scale-0 rounded-full transition-transform group-hover:scale-x-[150%] group-hover:scale-y-[220%]  px-4 text-[14px] bg-white flex justify-center items-center gap-2 flex whitespace-nowrap no-underline py-1.5 leading-[1.5]"></span></a>

#### Remote Server Connection

Add this to your Kiro MCP config file (`.kiro/settings/mcp.json` in your workspace or `~/.kiro/settings/mcp.json` for global).
